...
* **Exploratory data analysis** is a lost art. People seem ingrained to seek the statistical test that gives them an arbitrary p-value. One should actually explore the data, check for outliers and understand how the data is distributed. Do not naively apply a commonly used statistical test without any preliminary analysis. A counter argument, however, is that too much exploration could lead to "fishing" for patterns in the data. Make no mistake, exploring the data is definitely necessary to even begin making sense of it, especially if the data is too large and complex to decipher by eye. There has to be a balance, however, and it also depends on a researcher's integrity.
